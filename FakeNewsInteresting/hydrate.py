...
def hydrateTweets(
    tweetList, directory, directoryIfFailed, retryFailed=False, batch=1, pointer=0
):
...
def fetchOfficialCommunication(handle):
    ...
    formateStart = (
        str(year) + "-" + datetime.date(year, month, 1).strftime("%m") + "-01"
    )
    formateEnd = (
        str(year)
        + "-"
    ...
    for year in range(2020, 2023):
        for month in range(1, 13):
            formatDate = datetime.date(year, month, 1).strftime("%b%y")
            targetDir = (
                "../../../Documents/PS928Local/officalcomm/" + handle + "/" + formatDate
            )
            if not os.path.exists(targetDir):
                os.makedirs(targetDir)
